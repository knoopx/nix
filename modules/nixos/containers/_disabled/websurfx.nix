{pkgs, ...}: let
  root = "/mnt/storage/websurfx";

  config = pkgs.writeTextFile {
    name = "websurfx-config.lua";
    text = ''
      -- ### General ###
      ...
      -- simple-frosted-glow
      -- }}
      animation = "nil" -- the animation name which should be used with the theme or `nil` if you don't want any animations.
...
in {
  virtualisation.oci-containers.containers.search = {
    ...
    volumes = [
      "${config}:/etc/xdg/websurfx/config.lua"
    ];
